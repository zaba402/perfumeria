(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('perfumeriaApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-perfumeriaApp-error');
                var entityKey = httpResponse.headers('X-perfumeriaApp-params');
                if (errorHeader) {
